...
/// Like [`list_projects`], but additionally synthesises a *virtual* project for
/// every task workspace that no registered project owns. This makes the desktop
/// behave like the session monitor: instead of requiring the user to register a
/// project before a task can show up, any task on disk surfaces under a project
/// derived from its own `workspace`.
///
/// Registered projects are returned verbatim and first; virtual projects are
/// appended. A virtual project carries the same id the unregistered tasks
/// already store (so the frontend's `task.projectId === selectedProjectId`
/// filter matches), is named after the workspace's final path component, and
/// gets the default kanban columns so the UI renders it like any other project.
pub fn list_projects_discovered() -> Vec<Project> {
    let registered = list_projects();
    let task_refs: Vec<(String, Option<PathBuf>)> = crate::task::list_tasks(None)
        .into_iter()
        .map(|t| (t.project_id, t.workspace))
        .collect();
    merge_discovered_projects(registered, &task_refs)
}

/// Pure merge step behind [`list_projects_discovered`] — split out so it can be
/// unit-tested without touching the on-disk task store. `task_refs` is the
/// `(project_id, workspace)` of every task on disk.
pub fn merge_discovered_projects(
    registered: Vec<Project>,
    task_refs: &[(String, Option<PathBuf>)],
) -> Vec<Project> {
    use std::collections::HashSet;

    let registered_ids: HashSet<String> = registered.iter().map(|p| p.id.clone()).collect();
    // Canonical workspaces already owned by a registered project — so a legacy
    // task whose project_id is stale but whose workspace is now registered
    // doesn't spawn a confusing duplicate virtual project.
    let registered_ws: HashSet<PathBuf> = registered
        .iter()
        .filter_map(|p| Path::new(&p.workspace).canonicalize().ok())
        .collect();

    let mut out = registered;
    let mut synthesised: HashSet<String> = HashSet::new();

    for (project_id, workspace) in task_refs {
        if registered_ids.contains(project_id) {
            continue; // task belongs to a registered project already
        }
        let Some(ws) = workspace else {
            continue; // task never stamped a workspace → nothing to derive
        };
        // A registered project owns this dir (legacy stale project_id) — skip
        // rather than surface a duplicate of the registered project.
        if let Ok(canon) = ws.canonicalize() {
            if registered_ws.contains(&canon) {
                continue;
            }
        }
        if !synthesised.insert(project_id.clone()) {
            continue; // already synthesised this virtual project
        }
        let name = ws
            .file_name()
...
mod tests {
    ...
    fn make_project(id: &str, workspace: &str) -> Project {
        Project {
            id: id.into(),
            name: id.into(),
            workspace: workspace.into(),
    ...
    #[test]
    fn discovery_synthesises_virtual_project_for_unregistered_task() {
        let ws = tempfile::TempDir::new().unwrap();
        let registered = vec![make_project("reg-1", "/some/registered/ws")];
        let task_refs = vec![("auto-deadbeef".to_string(), Some(ws.path().to_path_buf()))];

        let out = merge_discovered_projects(registered, &task_refs);

        assert_eq!(out.len(), 2, "registered + 1 virtual");
        let virt = out.iter().find(|p| p.id == "auto-deadbeef").unwrap();
        // Named after the workspace's final path component.
        assert_eq!(virt.name, ws.path().file_name().unwrap().to_string_lossy());
        assert_eq!(virt.workspace, ws.path().to_string_lossy());
        assert_eq!(virt.kanban_columns.len(), default_kanban_columns().len());
    }

    #[test]
    fn discovery_skips_tasks_of_registered_projects() {
        let registered = vec![make_project("reg-1", "/some/ws")];
        let task_refs = vec![("reg-1".to_string(), Some(PathBuf::from("/some/ws")))];
        let out = merge_discovered_projects(registered, &task_refs);
        assert_eq!(out.len(), 1, "no virtual project for a registered task");
    }

    #[test]
    fn discovery_dedups_virtual_projects_by_id() {
        let ws = tempfile::TempDir::new().unwrap();
        let registered = vec![];
        let task_refs = vec![
            ("auto-1".to_string(), Some(ws.path().to_path_buf())),
            ("auto-1".to_string(), Some(ws.path().to_path_buf())),
        ];
        let out = merge_discovered_projects(registered, &task_refs);
        assert_eq!(out.len(), 1, "two tasks, same id → one virtual project");
    }

    #[test]
    fn discovery_skips_tasks_without_workspace() {
        let registered = vec![];
        let task_refs = vec![("auto-2".to_string(), None)];
        let out = merge_discovered_projects(registered, &task_refs);
        assert!(out.is_empty(), "no workspace → nothing to synthesise");
    }

    #[test]
    fn discovery_skips_workspace_owned_by_registered_project() {
        // A legacy task carries a stale project_id, but its workspace is now
        // owned by a registered project — must not surface a duplicate.
        let ws = tempfile::TempDir::new().unwrap();
        let registered = vec![make_project("reg-1", &ws.path().to_string_lossy())];
        let task_refs = vec![("fleet-task-local".to_string(), Some(ws.path().to_path_buf()))];
        let out = merge_discovered_projects(registered, &task_refs);
        assert_eq!(out.len(), 1, "registered project already owns this dir");
    }
}
