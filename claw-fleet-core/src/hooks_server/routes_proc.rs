...
pub(crate) fn route_procs(
    ...
    let body = serde_json::to_string(&crate::proc_runner::list_procs()).unwrap_or_default();
    let _ = request.respond(tiny_http::Response::from_string(body).with_header(json_header));
}
...
pub(crate) fn route_proc_input(
    ...
    match serde_json::from_slice::<crate::proc_runner::ProcInputRequest>(&body_bytes) {
        Ok(req_body) => match crate::proc_runner::proc_input(&req_body.id, &req_body.data_b64) {
            Ok(()) => {
                let _ = request.respond(
...
pub(crate) fn route_proc_resize(
    ...
    match serde_json::from_slice::<crate::proc_runner::ProcResizeRequest>(&body_bytes) {
        Ok(req_body) => {
            match crate::proc_runner::proc_resize(&req_body.id, req_body.cols, req_body.rows) {
                Ok(()) => {
                    let _ = request.respond(
                        tiny_http::Response::from_string(r#"{"ok":true}"#).with_header(json_header),
                    );
    ...
    match serde_json::from_slice::<crate::proc_runner::ClearProcRequest>(&body_bytes) {
        Ok(req_body) => {
            let result = match req_body.id {
                ...
                None => {
                    crate::proc_runner::clear_finished_procs(req_body.workspace_path.as_deref())
                }
            };
            match result {
                Ok(cleared) => {
