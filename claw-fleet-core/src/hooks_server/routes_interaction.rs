...
pub(crate) fn route_apply_session_title_guidance(
    ctx: &ServeCtx,
    mut request: tiny_http::Request,
    query: &std::collections::HashMap<String, String>,
    json_header: tiny_http::Header,
    ...
    match serde_json::from_slice::<Req>(&body_bytes) {
        Ok(req_body) => {
            match crate::session_title_guidance::apply_session_title_guidance(
                &req_body.user_title,
                &req_body.locale,
            ) {
                Ok(()) => {
...
pub(crate) fn route_remove_session_title_guidance(
    ctx: &ServeCtx,
    request: tiny_http::Request,
    query: &std::collections::HashMap<String, String>,
    json_header: tiny_http::Header,
    ...
    match crate::session_title_guidance::remove_session_title_guidance() {
        Ok(()) => {
            let _ = request.respond(
                tiny_http::Response::from_string(r#"{"ok":true}"#).with_header(json_header),
            );
...
pub(crate) fn route_test_decision_end_to_end(
    ...
    match crate::interaction_mode_test::run_end_to_end_test(std::time::Duration::from_secs(10)) {
        Ok(r) => {
...
pub(crate) fn route_test_decision_via_claude_cli(
    ...
    match crate::interaction_mode_test::run_claude_cli_test(std::time::Duration::from_secs(60)) {
        Ok(r) => {
            let body = serde_json::to_string(&r).unwrap_or_default();
            let _ =
                request.respond(tiny_http::Response::from_string(body).with_header(json_header));
...
pub(crate) fn route_apply_prd_mode(
    ...
    match serde_json::from_slice::<Req>(&body_bytes) {
        Ok(req_body) => {
            let result =
                prd_discipline::apply_prd_discipline(&req_body.user_title, &req_body.locale)
