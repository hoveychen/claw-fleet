...
/// 直连现状 —— 已配地址、地址的问题、token 在不在、能不能出码。
pub(crate) fn route_direct_host_status(
    _ctx: &ServeCtx,
    request: tiny_http::Request,
    ...
    let body = serde_json::to_string(&crate::direct_host::status()).unwrap_or_default();
    let _ = request.respond(tiny_http::Response::from_string(body).with_header(json_header));
}

/// 保存这台主机对手机而言的地址。回一份新现状,界面直接用它渲染。
pub(crate) fn route_direct_host_set(
    _ctx: &ServeCtx,
    mut request: tiny_http::Request,
    _query: &std::collections::HashMap<String, String>,
    json_header: tiny_http::Header,
    ...
    let parsed = serde_json::from_slice::<serde_json::Value>(&buf).unwrap_or_default();
    let field = |k: &str| {
        parsed.get(k).and_then(|s| s.as_str()).unwrap_or("").to_string()
    };
    let (base_url, token) = (field("baseUrl"), field("token"));
    let (status, body) = match crate::direct_host::set_config(&base_url, &token) {
        Ok(st) => (200, serde_json::to_string(&st).unwrap_or_default()),
...
/// 直连链接的二维码。信封与中转那张码一致(`{svg}`),好让两边的客户端代码同形。
pub(crate) fn route_direct_host_qr(
    _ctx: &ServeCtx,
    request: tiny_http::Request,
    _query: &std::collections::HashMap<String, String>,
    json_header: tiny_http::Header,
    ...
    let (status, body) = match crate::direct_host::direct_qr_svg_here() {
...
/// 同一条链接的文本形式(给复制按钮)。
pub(crate) fn route_direct_host_url(
    _ctx: &ServeCtx,
    request: tiny_http::Request,
    _query: &std::collections::HashMap<String, String>,
    json_header: tiny_http::Header,
    ...
    let (status, body) = match crate::direct_host::direct_url_text_here() {
