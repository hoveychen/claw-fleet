...
fn handle_request(
...
) {
    {
        ...
        match path {
            ...
            crate::routes::SKILL_AUTOSYNC => route_skill_autosync(ctx, request, &query, json_header, path),
            ...
            crate::routes::GIT_CLONE_STREAM if request.method() == &tiny_http::Method::Post => route_git_clone_stream(ctx, request, json_header),
            ...
            crate::routes::BROWSE_PATHS_ADD | crate::routes::BROWSE_PATHS_REMOVE
                if request.method() == &tiny_http::Method::Post => route_browse_paths_mutate(ctx, request, json_header, path),
