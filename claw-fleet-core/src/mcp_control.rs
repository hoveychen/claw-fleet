...
fn notes_tool_def() -> Value {
    json!({
        "name": "fleet__notes",
        "description": "Private checkpoint notes that survive context compaction and handoff relays (Fleet's local analogue of Codex's `notes` tool). For any task that may outlive one context window, keep `checkpoint.md` as a CURRENT-STATE document — goal, decisions taken, traps, progress, next step, and pointers (file paths, `fleet__history` line numbers) — and keep it current: `edit` the lines that changed, or `write` the whole file anew; delete entries that are finished or superseded instead of appending a log (git and the transcript already hold the history). Keep it under ~3,000 bytes: after a compaction Fleet re-injects checkpoint.md, and only that much fits whole. Other paths are for reference material you read on demand. Paths are virtual (relative, `/`-separated, no `.`/`..`); every file stays ≤ 1,000,000 bytes. Reads see this session's notes plus its handoff predecessors'; writes go to this session only (a successor's first edit/append of an inherited path starts from the predecessor's copy — rewrite it for the current state). Internal bookkeeping: do not narrate notes to the user. Actions: write (replace whole file), edit (replace one exact occurrence of old_text with new_text), append, read (optional start_line/stop_line, 1-based inclusive, negative counts from the end), list (optional prefix), search (case-sensitive literal substring).",
        "inputSchema": {
            "type": "object",
            "properties": {
...
fn artifact_tool_def() -> Value {
    json!({
        ...
        "description": "Store a finished DELIVERABLE in the Fleet artifact library (the desktop's 产出 page) so the user can find it, preview it and hand it to someone. Use this instead of the `fleet artifact` CLI. This is for files whose point is to be given to a person — a PDF, a slide deck, a spreadsheet, a rendered video or image, an exported dataset, and equally an outward-facing html report or a markdown spec you are delivering. The split against `fleet__wiki` is AUDIENCE, not file format: there is no format filter here (`add` takes any single file), so pick by whether the thing is being handed over (here) or banked for later sessions to read back (`fleet__wiki`). NOT for source code — that lives in the repo. Add the artifact as soon as you produce it: the file is usually written inside a `.worktrees/<task-id>` checkout that gets deleted when the plan merges, and once that happens the deliverable is gone. Actions: add (--path required; --title/--note optional and worth filling in — they are what the user reads on the card), list, get (--id required), delete (--id required). `add` reads a path on the host serving this session.",
        "inputSchema": {
            "type": "object",
            "properties": {
