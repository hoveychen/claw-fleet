...
mod tests {
    ...
    /// a dangling "你继承了这些 watch" (you inherited these watches) header for an empty list.
    #[test]
...
**Plan:** Relay\n- [ ] **P1** — a\n<!-- fleet:prd:end id=\"relay\" -->\n",
    ...
    /// "回到这个 chain 最开始我的问题" (back to my original question at the start of the chain) and answered from the plan it was handed
    ...
    fn successor_prompt_carries_the_chain_before_it() {
        ...
        // contains the words "接力链" (handoff chain) in an unrelated sentence.)
    ...
    fn render_chain_shows_origin_and_attributes_notes_to_the_hop_that_wrote_them() {
        ...
        // Anchor on the hop *headers* (line-initial) — "第 2 棒" (2nd relay) also occurs
        // inside hop 1's "交给第 2 棒时留下的 note" (note left for 2nd relay) lead-in.
        let h1 = full.find("\n第 1 棒").unwrap();
        let h2 = full.find("\n第 2 棒").unwrap();
