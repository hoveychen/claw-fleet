...
// ── Repository overview (mobile "仓库" (Repository) surface) ────────────────────
//
...
pub struct WorktreeHealth {
    ...
    pub dirty_count: usize,
    /// The uncommitted entries themselves, so the "脏 N" (Dirty N) badge can expand into
...
pub struct RepoDetail {
    ...
    pub dirty_count: usize,
    /// The uncommitted entries themselves, so the main-checkout "脏 N" (Dirty N) badge can
