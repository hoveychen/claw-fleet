...
impl AgentSource for ClaudeCodeSource {
    ...
    fn spawn(
        ...
        // Empty entrypoint → the "新会话" (new session) default, so a bare spawn is still
...
fn reconcile_claude_liveness(sessions: &mut [SessionInfo], processes: &[CliProcess]) -> bool {
    ...
    for s in sessions.iter_mut() {
        ...
        // is gone — downgrade it so the "会话运行中" (session running) UI, the pending-message
...
/// `!proc_alive`: the "会话运行中" (session running) enqueue UI (`canEnqueueSession`), the
