...
impl TaskOutcome {
    ...
    /// What the agent is told when the user ended the task from a card. Shared
    /// by every card surface that carries a terminal button (`fleet__ask` via
    /// the MCP handler, `AskUserQuestion` via the elicitation hook) so the two
    /// wordings cannot drift — the guidance files quote these strings verbatim.
    pub fn terminal_notice(self) -> &'static str {
        match self {
            TaskOutcome::Completed => {
                "TASK FINISHED — the user pressed 「结束任务」 and closed this task as complete. \
            ...
            TaskOutcome::Abandoned => {
                "TASK ABANDONED — the user pressed 「放弃任务」 and closed this task as \
