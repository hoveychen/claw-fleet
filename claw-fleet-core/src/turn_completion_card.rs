...
fn deliver_reminder(job: &TurnCardJob, resp: &ElicitationResponse) {
    ...
    // The session can register a relay *while* the card sits in the panel (the
    // card is raised at turn end, the answer can land minutes later). Resuming
    // it then wakes a retired predecessor next to its live successor — two
    // sessions on one transcript, and the predecessor's context ends at "I just
    // registered a handoff", so it re-registers the same stale baton.
