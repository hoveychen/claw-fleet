...
pub fn render_hint_in(root: &Path, session_id: &str, readable: &[String]) -> Option<String> {
    ...
    if let Some(hidden) = files
        .len()
        .checked_sub(MAX_HINT_ROSTER_ROWS)
        .filter(|n| *n > 0)
    {
...
mod tests {
    ...
    fn hint_keeps_the_tail_of_an_appended_checkpoint() {
        ...
        assert!(hint.len() <= MAX_HINT_BYTES, "hint is {} bytes", hint.len());
        assert!(
            hint.contains("LAST_ENTRY"),
            "newest entry was clipped away: {hint}"
        );
        assert!(
            !hint.contains("FIRST_ENTRY"),
            "oldest entry survived instead of the newest"
        );
