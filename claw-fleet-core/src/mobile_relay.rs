...
// The image squeeze loop lives in `fleet-image`, not here. It is the one
// per-pixel hot path in Fleet, and `image`'s generic code is monomorphized into
// whichever crate calls it — so while it sat in this file, the *whole* workspace
// had to build at `opt-level = 1` to keep the phone's 15s `tail` timeout. Its
// own crate can be pinned to `opt-level = 3` on its own; see the comment above
// `[profile.dev]` in the root `Cargo.toml`.
use fleet_image::{
    downscale_decision_asset, downscale_image, DECISION_ASSET_HARD_CAP_BYTES,
    DECISION_ASSET_MIN_DIM, DECISION_ASSET_TARGET_BYTES,
};
