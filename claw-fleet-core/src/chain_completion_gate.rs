...
//!
//! This module does not judge whether the work is done — it cannot. It bounces
//! the call **once per session**, naming the hop position and quoting the
//! chain's finish line, so the agent re-reads it before the boss ever sees a
//! success button. Re-sending the same card goes through untouched: an agent
//! that has looked and still believes it is done is exactly the case we want to
//! allow.
//!
//! "The finish line" is [`crate::handoff::HandoffChain::goal`] when the chain
//! has one — a sentence somebody wrote *as* the objective. Chains recorded
//! before goals existed, and exploratory chains that legitimately have none,
//! fall back to the opening of hop 1's note, which is where the objective used
//! to live as prose.
...
/// Build the refusal text for a session at `hop` of `chain_len`.
///
/// Quotes the chain's stated `goal` when it has one — that is a sentence
/// somebody wrote *as* the finish line, so it is worth far more than a guess at
/// one. Chains recorded before goals existed, and exploratory chains that
/// legitimately have none, fall back to the opening of hop 1's note.
///
/// Split out from [`refusal_for`] so the wording can be tested without a
/// `~/.fleet` on disk.
pub fn refusal_text(hop: u32, chain_len: u32, goal: Option<&str>, first_note: &str) -> String {
    let (label, body, how_to_check) = match goal {
        Some(g) => (
            "本链的目标是",
            g.trim().to_string(),
            "现在做一件事：确认这个目标真的达成了。\n".to_string(),
        ...
        None => (
            "这条链没有写明目标，第 1 棒的 note 是这么开头的",
            excerpt(first_note, ORIGIN_EXCERPT_CHARS),
            "现在做一件事：确认老板最初要的那件事真的完成了。不确定就 `fleet__handoff` 传 \
             `action=\"show\"` 把整条链读一遍。\n"
                .to_string(),
        ),
    };
    format!(
        "你在接力链的第 {hop}/{chain_len} 棒，却把这张卡标成了 `taskComplete: true`，\
         ...
         收工的判据是**链的目标**达成，不是你手上这个 plan 做完 —— \
         计划树全勾不等于老板要的那件事做完了。\n\n\
         {label}：\n\n> {body}\n\n\
         {how_to_check}\
         - **确认已完成** → 把这张卡原样再发一次，`taskComplete` 保持 true，本会话不会再拦第二次。\n\
         - **没完成** → 把 `taskComplete` 改成 false，并在卡里点名还剩什么、下一步该干哪件。\n\
         - **目标本身已经不成立**（老板改了路线）→ 用 `fleet__handoff` 的 `goal` + `goalReason` \
         显式改掉它，别拿一张成功卡把旧目标糊过去。",
        hop = hop,
        chain_len = chain_len,
        body = body.replace('\n', "\n> "),
    )
...
mod tests {
    ...
    fn refusal_names_the_hop_and_quotes_the_origin() {
        let text = refusal_text(26, 26, None, "# 接力简报\n\n老板要把 27 页前端一次性全重写");
        assert!(
    ...
    #[test]
    fn a_stated_goal_is_quoted_instead_of_the_note() {
        ...
        assert!(text.contains("本链的目标是"), "{text}");
        assert!(text.contains("整个二期上线"), "{text}");
        assert!(
            !text.contains("某段无关的开场白"),
            "the note is not quoted too: {text}"
        );
        assert!(
            text.contains("goalReason"),
            "offers the revise-it escape hatch: {text}"
        );
    }
