...
/// Method names generated by the `remote_delegations!` macro table in
/// `remote.rs`. These are real `Backend` overrides on RemoteBackend, but they
/// appear as macro *entries* (`name -> Ret = ROUTE;` / `name = ROUTE;`) rather
/// than literal `fn name(` declarations, so `impl_methods` / `top_level_fns`
/// (which scan for `fn NAME(`) never sees them. Check A must union these in or
/// it would flag every macro-generated method as a missing remote override.
/// Returns an empty set until the macro is introduced, so the guard stays green
/// through a partial migration.
fn remote_macro_methods() -> HashSet<String> {
    let src = read(REMOTE_RS);
    let mut out = HashSet::new();
    let Some(bang) = src.find("remote_delegations!") else {
        return out;
    };
    let (start, end) = block_after(&src, bang);
    let block = &src[start..=end];
    // Entry forms: `name -> Ret = ROUTE;` (getters) and `name = ROUTE;` (posts).
    // The return type never contains `=`/`;`/`{`/`}`, so a single regex covers
    // both: capture the leading ident before an optional `-> Ret` and `= CONST;`.
    let entry = Regex::new(
        r"([a-z_][a-z0-9_]*)\s*(?:->[^=;{}]*)?=\s*(?:claw_fleet_core::routes::)?[A-Z][A-Z0-9_]*\s*;",
    )
    .unwrap();
    for c in entry.captures_iter(block) {
...
fn check_a_remote_impl_covers_local_default_methods() {
    ...
    let mut remote = impl_methods(REMOTE_RS, "RemoteBackend");
    // Fold in methods generated by the `remote_delegations!` macro table, which
    // are real overrides but invisible to the `fn NAME(` scanner.
    remote.extend(remote_macro_methods());
