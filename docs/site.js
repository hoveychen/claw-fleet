...
(() => {
  const script = document.currentScript || [...document.scripts].find(el => /site\.js(?:\?|$)/.test(el.src));
  ...
  function fromHash() {
    // Preserve the existing README / field-guide #demo link.
    if (location.hash === '#demo') document.querySelector('#explore').scrollIntoView();
  ...
  form.addEventListener('submit', event => {
    ...
    submit.hidden = true; reset.hidden = false; reset.focus();
  });
  form.addEventListener('reset', () => {
    status.textContent = ''; form.querySelector('fieldset').disabled = false;
    submit.hidden = false; reset.hidden = true; submit.focus();
  });
  ...
  fetch(manifestURL, {signal: AbortSignal.timeout(5000)})
    ...
    .then(manifest => {
      if (manifest.schema !== 1 || typeof manifest.version !== 'string' || !manifest.china) return;
      const urls = new Map();
      ...
      note.textContent = note.dataset.ready.replace('{version}', manifest.version);
      const changeSource = () => links.forEach(link => {
        link.href = selector.value === 'china' ? urls.get(link) : originals.get(link);
        link.parentElement.querySelector('.fallback').hidden = selector.value !== 'china';
      });
      selector.addEventListener('change', changeSource);
      if (document.body.dataset.locale === 'zh') selector.value = 'china';
      changeSource();
    }).catch(() => { /* GitHub remains available. */ });
})();
