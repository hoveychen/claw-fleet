...
- **dsh 后端已真跑（2026-09-20，第 4 棒，`dsh_explain::dsh_fork_ask`，父会话 session-99153729 / deepseek-official/deepseek-flash effort low，两次）**：`session/fork` → 子会话 → `session/prompt` → 跟 follow 流到 `turn/end`，7.1～9.4 秒回答；用量 cache_read 21248 / miss 2824 / output 646，按 DeepSeek 表价 $0.0017 入账。父会话 zstd 文件 md5 前后一致；`~/.dsh/settings.yaml` 的 `agent-default-model` 不变（父模型恰等于全局默认，`selectModel` 对齐与恢复那条路径**没被走到**，待一个父模型≠默认的会话再验）；子会话 id 写入 `~/.fleet/explain/forks/`，`fleet` 会话列表看不到它。**dsh 0.1.5-rc.1 的 follow 流上已没有 `assistant/chunk`**（整份 dsh 源码里该事件名已消失；dump 出的 13 帧只有 turn/start → step/start → request/header → assistant/message → step/end → turn/end），所以 dsh 这一路和 codex 一样**没有逐字流式**，答案在 `assistant/message` 落地时整段出现。可能的补救是轮询 `session/page` 尾页里的 in-flight partial（`fetch_history` 文档提到尾页带 partial），留作后续。原始帧 dump 开关：`FLEET_EXPLAIN_DSH_DUMP=<file>`。
