...
for lang, scene in scenes.items():
    ...
    for i, title in enumerate(scene['artifacts']):
        ...
        if i in (1,5):
            content = f'<div class="metric">{"+18.6%" if i==1 else "+12.0%"}</div><p>{("Revenue growth · September" if i==1 else "Activation lift · Variant B") if lang=="en" else ("九月收入增长" if i==1 else "B 方案激活率提升")}</p><svg viewBox="0 0 620 230" width="100%"><path d="M10 205 L110 177 L210 184 L310 122 L410 137 L510 65 L610 20 L610 230 L10 230Z" fill="{accent}" opacity=".18"/><path d="M10 205 L110 177 L210 184 L310 122 L410 137 L510 65 L610 20" fill="none" stroke="{accent}" stroke-width="5"/></svg><div class="row"><span>APR</span><span>MAY</span><span>JUN</span><span>JUL</span><span>AUG</span><span>SEP</span></div>'
        elif i == 2:
            content = f'<div class="metric" style="font-family:Georgia">{"Field<br>notes." if lang=="en" else "田野<br>笔记。"}</div><div class="swatches"><b style="background:#274b39"></b><b style="background:#c26b42"></b><b style="background:#e5c693"></b></div><p>{"A quieter kind of confidence." if lang=="en" else "安静，也有自己的态度。"}</p>'
        elif i == 6:
            content = f'<div class="metric" style="font-size:82px">{"Make room<br>for the work." if lang=="en" else "让好想法<br>成为作品。"}</div><hr><p>{"AUTUMN / 2026 — STUDIO NOTES 01" if lang=="en" else "2026 秋季 — 工作室手记 01"}</p>'
        elif i == 7:
