...
## Task-as-Unit V1: P-item worker execution constraints
...
- **Don't run phase-level commands.** No `cargo build` / `cargo test` / `pnpm build` / `playwright test` — those are reserved for phase P-items at the tail of the DAG. The most you may run is `cargo check --package <crate>` to validate one crate compiles after your edit.
- **Don't `git commit` or change branches.** The Master orchestrates branches; you operate on `fleet/<slug>` already checked out for you.
