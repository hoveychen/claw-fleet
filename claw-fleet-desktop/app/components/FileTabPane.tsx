 ...
 * was written for the "仓库" (Files) page's out-of-tree case and wore its assumptions
 ...
 * was about to perform returns one; and it drew its own "关闭" (Close) button beside the
 * rail card's ✕, which was the same action twice.
 *
 * The reader itself is still the shared `FilePreview`, so a file looks the same
 * here as on the "仓库" (Files) page. Only the chrome is the rail's own — the density line
...
export function FileTabPane({
  ...
  doc: AuxDoc;
  tail: AuxCardTail;
  /** The session's repo, needed to hand the path to the "仓库" (Files) page. Blank on a
