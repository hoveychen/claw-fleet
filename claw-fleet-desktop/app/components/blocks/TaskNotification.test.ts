...
<tool-use-id>toolu_01NDLwyyHSzByvFgw7u4fq2T</tool-use-id>
...
describe("parseTaskNotification", () => {
  it("lifts every field out of the envelope", () => {
    const p = parseTaskNotification(SAMPLE);
    expect(p).not.toBeNull();
    expect(p!.taskId).toBe("a1389701e02753075");
    expect(p!.toolUseId).toBe("toolu_01NDLwyyHSzByvFgw7u4fq2T");
    ...
    expect(p!.note).toContain("fires each time");
    // The multi-line markdown body survives intact.
  ...
  it("returns null when the tag has no payload fields", () => {
    expect(parseTaskNotification("<task-notification></task-notification>")).toBeNull();
  });

  it("survives a notice that carries only a status", () => {
    const p = parseTaskNotification(
