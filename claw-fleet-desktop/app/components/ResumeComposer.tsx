...
export function ResumeComposer({
  ...
  // sessions snapshot on success, but the round-trip (write + re-enrich + emit
  // + re-render of the whole session list) is long enough that a chip that only
  // disappears on the snapshot reads as a dead button. So hide it optimistically
  // — keyed by position+text, the same shape the mobile composer uses — and
  // un-hide if the call fails.
  const [cancelledKeys, setCancelledKeys] = useState<Set<string>>(new Set());
  // A fresh snapshot is authoritative: drop the optimistic hides so a *new*
  // enqueue that happens to land on the same position+text isn't swallowed.
  ...
  const handleCancelQueued = async (index: number) => {
    ...
    const key = `${index}:${pendingMessages[index] ?? ""}`;
