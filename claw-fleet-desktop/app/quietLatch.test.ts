...
describe("quiet-alive latch (anti-flicker hysteresis)", () => {
  ...
  it("a session that was never quiet is not latched", () => {
    ...
    expect(stickyQuiet(st, "s1", { rawQuiet: false, lastActivityMs: t0, now: t0 })).toBe(false);
  });

  it("raw quiet latches immediately", () => {
    const st = createQuietLatch();
    expect(
      stickyQuiet(st, "s1", { rawQuiet: true, lastActivityMs: t0 - 60_000, now: t0 }),
    ).toBe(true);
  });

  it("one sparse write does NOT unlatch — this is the flicker", () => {
    const st = createQuietLatch();
    stickyQuiet(st, "s1", { rawQuiet: true, lastActivityMs: t0, now: t0 });
    ...
    expect(stickyQuiet(st, "s1", { rawQuiet: false, lastActivityMs: write, now: write })).toBe(
      true,
    );
    // …and it must still be faded once that window decays back to quiet.
    expect(
      stickyQuiet(st, "s1", { rawQuiet: true, lastActivityMs: write, now: write + 40_000 }),
    ).toBe(true);
  });
  ...
  it("two writes close together unlatch — the session is genuinely working again", () => {
    ...
    stickyQuiet(st, "s1", { rawQuiet: false, lastActivityMs: first, now: first });
    const second = first + DENSE_WRITE_MS - 1_000;
    expect(stickyQuiet(st, "s1", { rawQuiet: false, lastActivityMs: second, now: second })).toBe(
      false,
    );
    // Once unlatched it stays unlatched while the writes keep coming.
    const third = second + 5_000;
    expect(stickyQuiet(st, "s1", { rawQuiet: false, lastActivityMs: third, now: third })).toBe(
      false,
    );
  });
  ...
  it("sparse writes keep re-arming the latch instead of accumulating into a recovery", () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      expect(stickyQuiet(st, "s1", { rawQuiet: false, lastActivityMs: at, now: at })).toBe(true);
    }
  });
  ...
  it("repeated observations of the same timestamp are idempotent", () => {
    ...
    for (let i = 0; i < 4; i++) {
      expect(
        stickyQuiet(st, "s1", { rawQuiet: false, lastActivityMs: write, now: write + i * 1_000 }),
      ).toBe(true);
    }
  });
  ...
  it("latches are per session", () => {
    const st = createQuietLatch();
    stickyQuiet(st, "s1", { rawQuiet: true, lastActivityMs: t0, now: t0 });
    expect(stickyQuiet(st, "s2", { rawQuiet: false, lastActivityMs: t0, now: t0 })).toBe(false);
  });
});
