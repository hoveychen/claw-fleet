...
let mockRemoteWorkspaces: {
  ...
  // Host-book entry on a path the mock sessions actually run in, so the session
  // card's remote badge is visible under ?mock.
  ...
  // renders its probe/install/login affordances under ?mock. Also stands in for
  // the "registered before the host book existed" shape the settings page
  // lists separately.
