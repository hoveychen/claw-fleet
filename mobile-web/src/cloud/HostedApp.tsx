...
function route() { const parts = location.pathname.split("/").filter(Boolean); const project = parts[0] === "project" ? parts[1] : undefined; const task = parts[2] === "tasks" ? parts[3] : undefined; const view: CloudView | undefined = parts[2] === "operations" ? "operations" : parts[2] === "decisions" ? "decision_inbox" : undefined; return { project, task, view }; }
...
export function HostedApp({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  return <CloudWorkspace client={client} projectId={projectId} initialTaskId={initial.task} initialView={initial.view} onNavigate={(taskId, view: CloudView = "tasks") => { const suffix = view === "decision_inbox" ? "/decisions" : view === "operations" ? "/operations" : ""; const path = taskId ? `/project/${encodeURIComponent(projectId)}/tasks/${encodeURIComponent(taskId)}` : `/project/${encodeURIComponent(projectId)}${suffix}`; history.pushState({}, "", path); }} />;
}
