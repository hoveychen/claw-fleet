...
/// Install whatever the control plane is missing, every time `fleet webui`
/// starts.
///
...
/// The documented Linux quickstart is curl, chmod, `fleet webui` — so that path
/// has to arrive at a governed host on its own.
///
/// **Why every start, and why that does not trample the user's choices.** The
/// obvious alternative — install once, remember it in a marker file — heals a
/// never-configured host but not one that lost its `~/.claude` afterwards
/// (a reset, a wiped container layer, a half-failed install). What makes
/// running every time safe is that a deliberate switch-off is now recorded
/// where this process can see it, in `~/.fleet/control-plane-prefs.json`
/// (`claw_fleet_core::control_plane_prefs`, written by the remove path itself).
/// So [`claw_fleet_core::control_plane::heal`] installs only what is *both*
/// missing and never turned off — "absent because nobody set it up" gets fixed,
/// "absent because the user said no" is left alone.
fn heal_control_plane() {
    let settings = super::bootstrap::resolve_settings(None, None, None);
    let steps = claw_fleet_core::control_plane::heal(&settings);
    if steps.is_empty() {
        return; // already whole — the common case, not worth a line of output
    }
    ...
    if !installed.is_empty() {
        eprintln!(
            "[fleet webui] installed missing control-plane pieces: {}. \
             Adjust them any time in the app's settings panel.",
            installed.join(", ")
        );
    }
    for s in steps.iter().filter(|s| s.result.is_err()) {
        if let Err(e) = &s.result {
            eprintln!("[fleet webui] could not install {}: {e}", s.name);
        }
    }
}
...
pub(crate) fn cmd_webui(
    ...
    // allow-list on top of that. See [`heal_control_plane`].
    heal_control_plane();
