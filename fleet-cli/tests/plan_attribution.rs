...
fn create_inherits_the_focused_plan_as_parent() {
    ...
    assert!(
        !out.status.success(),
        "a bare --root mid-plan must be refused"
    );
...
fn check_claims_focus_and_advances_current_task() {
    ...
    plan(
        home.path(),
        ws.path(),
        &["add", "alpha", "P1", "--text", "first"],
    );
    plan(
        home.path(),
        ws.path(),
        &["add", "alpha", "P2", "--text", "second"],
    );
    ...
    assert!(
        cur.contains("**P2**"),
        "current task should be P2, got {cur:?}"
    );
}
